...
def resource_path(relative_path):
    ...
    so we use its directory.  When running as a plain .py script we use
...
# ---- Default values for every INI key ----
# These are used both to seed a missing config file AND as
# fallback values when a key is absent from an existing file.
_INI_DEFAULTS = {
    "window": {
    ...
    "location_behavior": {
        # Auto-center map on player when /loc fires. Also toggleable live via SET panel.
        "auto_center_on_loc": "true",
        # Also reset zoom to 1.0 before centering.
        "auto_zoom_on_loc": "false",
    },
    "map_z_layers": {
        # Per-map Z range -> layer mapping. Format: mapname_Z_LayerName = min,max
        # Safe key = map name lowercased, spaces replaced with underscores.
        # LayerName must match the layer name in MAP_DEFINITIONS exactly.
        # Example (Halnir Cave safe key: halnir_cave):
        #   halnir_cave_Z_Upper = 0,9999
        #   halnir_cave_Z_Mid   = -300,0
        #   halnir_cave_Z_Lower = -9999,-300
    },
}
...
def _write_ini_with_comments(path: str):
    ...
    lines = []
    lines.append("; ================================================================")
    lines.append("; Pantheon Morte Map v4.0.0.0  —  User Configuration")
    ...
    comment_map = {
        ...
        "auto_center_on_loc": "; true = auto-center map on player when /loc fires",
        "auto_zoom_on_loc":   "; true = also reset zoom to 1.0 when auto-centering",
    }
    ...
    # Write the [map_z_layers] section -- no fixed keys, just instructions
    lines.append("[map_z_layers]")
    lines.append("; Per-map Z-value to layer selection.")
    lines.append("; When /loc fires and Z falls in a range, only that layer turns on.")
    lines.append("; Format:  <map_safe_key>_Z_<LayerName> = <min_z>,<max_z>")
    lines.append("; map_safe_key = map name lowercased, spaces with underscores.")
    lines.append("; LayerName    = must match layer name in MAP_DEFINITIONS exactly.")
    lines.append("; Example (Halnir Cave key: halnir_cave):")
    lines.append(";   halnir_cave_Z_Upper = 0,9999")
    lines.append(";   halnir_cave_Z_Mid   = -300,0")
    lines.append(";   halnir_cave_Z_Lower = -9999,-300")
    lines.append("; Tip: enable game_z_index in [jumploc] so Z is read from /loc.")
    lines.append("")
    lines.append("")
...
def _parse_hex_color(s: str, default: QColor) -> QColor:
    """Convert a #RRGGBB or #AARRGGBB string to a QColor."""
    s = (s or "").strip()
    if not s.startswith("#"):
...
def _build_theme(cfg: configparser.ConfigParser) -> dict:
    """Build a dict of key -> QColor from the [colors] section."""
    out = {}
...
TOP_BAR_HEIGHT   = _CFG.getint   ("window",               "top_bar_height", fallback=76)
...
def theme_q(key: str, default: QColor) -> QColor:
    """Return the QColor for a theme key, or default if the key is missing/invalid."""
    c = THEME.get(key)
...
# The key sequence string used for the fallback Qt shortcut.
# The REAL hotkey is registered globally via Windows API — see _GlobalHotkeyThread below.
TOGGLE_MAP_KEYS  = _CFG.get      ("keybinds", "toggle_map_visibility", fallback="Shift+M")

# Read a /jumploc token index from the INI.
MAP_X_I = _CFG.getint("jumploc", "map_x_index", fallback=1)
MAP_Y_I = _CFG.getint("jumploc", "map_y_index", fallback=3)
...
AUTO_CENTER_ON_LOC = _CFG.getboolean("location_behavior", "auto_center_on_loc", fallback=True)
AUTO_ZOOM_ON_LOC   = _CFG.getboolean("location_behavior", "auto_zoom_on_loc", fallback=False)


def _load_map_z_layer_ranges(cfg):
    """
    Build the Z-to-layer lookup table from [map_z_layers] in config.ini.
    Keys: halnir_cave_z_upper = -300,0  (configparser lowercases keys)
    Returns: { safe_map_key: [(layer_name, min_z, max_z), ...] }
    Called AFTER MAP_DEFINITIONS and _safe_key are defined.
    """
    result = {}
    if not cfg.has_section("map_z_layers"):
        return result
    for raw_key, val in cfg.items("map_z_layers"):
        # Split at "_z_" to separate the map safe-key from the layer name.
        # configparser lowercases everything so "Halnir_Cave_Z_Upper" -> "halnir_cave_z_upper"
        sep = "_z_"
        idx = raw_key.find(sep)
        if idx < 1:
            continue  # malformed key — skip silently
        safe_key_ini   = raw_key[:idx]
        layer_name_low = raw_key[idx + len(sep):]
        # Recover original-case layer name from MAP_DEFINITIONS
        matched = None
        for mname, layers in MAP_DEFINITIONS.items():
            if _safe_key(mname) == safe_key_ini:
                for ld in layers:
                    if ld["name"].lower() == layer_name_low:
                        matched = ld["name"]
                        break
        if matched is None:
            # User may have typed it manually — use as-is
            matched = layer_name_low
            
        # Parse the "min,max" value pair
        parts = val.split(",")
        if len(parts) != 2:
            print(f"[z_layers] Bad format for '{raw_key}' -- expected 'min,max'")
            continue
        try:
            min_z, max_z = float(parts[0].strip()), float(parts[1].strip())
        except ValueError:
...
# ==============================================================
# APP IDENTITY  (hard-coded — not in config.ini)
# ==============================================================
...
APP_FILENAME     = "Pantheon_Morte_Map[4.0.0.0].exe"
...
# ==============================================================
# MAP DEFINITIONS
# Each map has one or more layers (image files).
# The layer 'name' values MUST match what you put in [map_z_layers]
# in config.ini exactly (case-sensitive).
# ==============================================================
...
def _safe_key(map_name: str) -> str:
    """Convert a map name to a safe INI / filename key (lowercase, underscores)."""
    return map_name.lower().replace(" ", "_")

# Built after MAP_DEFINITIONS + _safe_key are both defined
...
# ==============================================================
# MATH — affine calibration transform
# ==============================================================
...
def world_to_pixel(map_x, map_y, xc, yc):
    """Convert game world coordinates to image pixel coordinates."""
...
def _jumploc_required_token_count() -> int:
    """Minimum number of tokens a valid /jumploc line must have."""
...
# ==============================================================
# SIGNALS
# Qt signals let background threads communicate safely with the
# main UI thread without crashing or race conditions.
# ==============================================================
class Signals(QObject):
    repaint_needed = pyqtSignal()           # ask canvas to redraw
    map_loaded     = pyqtSignal(str, list)  # background map load finished
    loc_updated    = pyqtSignal()           # /jumploc fired -> auto-center/zoom/layer
    flash_msg      = pyqtSignal(str)        # show flash from any thread
    hotkey_fired   = pyqtSignal()           # global Shift+M pressed



# ==============================================================
# GLOBAL HOTKEY THREAD
# Registers Shift+M (or whatever TOGGLE_MAP_KEYS says) system-wide
# via Windows RegisterHotKey so it fires even when the game has focus.
# ==============================================================

# Windows virtual key code table for letters and digits
...
def _parse_hotkey(key_seq):
    """Parse 'Shift+M' -> (modifiers_flag, vk_code). Returns (0,0) on failure."""
    parts = [p.strip() for p in key_seq.upper().split("+")]
    mods = 0; vk = 0
    for part in parts:
        if part == "SHIFT":               mods |= _MOD_SHIFT
        elif part in ("CTRL","CONTROL"):  mods |= _MOD_CTRL
        elif part == "ALT":               mods |= _MOD_ALT
        elif part in ("WIN","META"):       mods |= _MOD_WIN
        elif part in _VK_MAP:             vk = _VK_MAP[part]
    return mods, vk


class _GlobalHotkeyThread(threading.Thread):
    """
    Daemon thread registering a Windows system-wide hotkey.
    Emits sig.hotkey_fired (Qt main thread delivery) on press.
    Uses RegisterHotKey so it fires even when the game has focus --
    unlike QShortcut which only fires when our window is active.
    """
    HOTKEY_ID = 1  # arbitrary unique ID per process
    ...
    def run(self):
        ...
        # RegisterHotKey(hWnd=None means this thread's message queue)
        ...
        while not self._stop.is_set():
            # PeekMessage is non-blocking; PM_REMOVE=1 removes msg from queue
            if ctypes.windll.user32.PeekMessageW(ctypes.byref(msg), None, 0, 0, 1):
                ...
                import time as _t; _t.sleep(0.05)  # poll every 50 ms — very low CPU usage
...
# ==============================================================
# PANEL WIDGET
# Semi-transparent dark rounded background widget used for all
# side panels and the top bar.
# ==============================================================
class Panel(QWidget):
...
# ==============================================================
# MAP CANVAS
# The widget that fills the entire window behind the top bar.
# All drawing (map layers, player dot, pins, markers, cal points)
# happens in paintEvent.  Mouse events handle panning and clicking.
# ==============================================================
class MapCanvas(QWidget):
    def __init__(self, overlay):
        super().__init__(overlay)
        self.ov = overlay          # reference to the MapOverlay window
        self.setMouseTracking(True)
    
    # ----------------------------------------------------------
    # paintEvent — draws everything
    # ----------------------------------------------------------
    def paintEvent(self, event):
        ov = self.ov
        p = QPainter(self)
        ...
        # ---- Map layers ----
        # Find the topmost visible layer — it gets full opacity; others get LOWER_LAYER_OPACITY.
        ...
        lbl_shadow = QColor(0, 0, 0)  # shared drop-shadow colour for labels
        ...
        # ---- Last-click crosshair (ping marker) ----
        if ov.last_click_px:
        ...
        # ---- Named world markers (typed coordinates) ----
    ...
    def _draw_pin(self, p, img_px, img_py, name, highlight=False):
        """Draw a teardrop-shaped drop-pin with a name label above it."""
        ov = self.ov
        ...
        # Teardrop = circle + triangle merged
        path = QPainterPath()
        ...
        # Specular highlight inside pin head
        p.setPen(Qt.NoPen)
...
class MapOverlay(QMainWindow):
    ...
    def _on_map_loaded(self, map_name, images):
        ...
        Converts images (makes near-black pixels transparent), sets up layers,
        reloads calibration / pins / markers, then fits the map to fill the viewport.
        """
        self.map_layers    = []
        self.layer_visible = []
    ...
    def _schedule_geom_save(self):
        """Start/restart the debounce timer for geometry saves (avoids hammering the disk)."""
        if PERSIST_GEOMETRY:
    ...
    def _center_map_image(self):
        """Center the map image in the canvas at current zoom -- no zoom change."""
    ...
    def _build_ui(self):
        """Create all widgets, the top bar, all panels, and wire up signals."""
        ...
        # Canvas sits behind everything else
        ...
        def mk(text, bg, tip="", w=42):
            """Helper: create a styled toolbar button."""
            b = QPushButton(text)
            b.setFixedSize(w, 26)
            b.setToolTip(tip)
            b.setStyleSheet(
        ...
        # SET button opens the Settings panel with auto-center/auto-zoom checkboxes
        ...
        # Create a container for Coordinate readout labels (bottom row of top bar) 
        ...
        # Settings panel -- new in v4.0 -- auto-center/zoom checkboxes
    ...
    def _make_settings_panel(self, parent):
        """
        Settings side panel.  Contains:
          - Auto-Center on /loc  checkbox
          - Auto-Zoom to 1.0 on /loc  checkbox
        Both can be toggled live without restarting the app.
        """
        p = Panel(parent)
        v = QVBoxLayout(p)
        ...
        # Auto-center checkbox -- synced to self.auto_center
        ...
        # Auto-zoom checkbox -- synced to self.auto_zoom
    ...
    def _on_auto_center_changed(self, state):
        """Checkbox toggled -- update runtime flag immediately (no restart needed)."""
    ...
    def _toggle_settings_panel(self):
        """Open/close Settings panel; sync checkboxes to current state first."""
        vis = not self.settings_panel.isVisible()
        # Hide all other panels first
        self.layer_panel.hide(); self.calib_panel.hide()
        self.pin_panel.hide(); self.markers_panel.hide()
        if vis:
            self.chk_auto_center.setChecked(self.auto_center)
            self.chk_auto_zoom.setChecked(self.auto_zoom)
    ...
    def _make_layer_panel(self, parent):
        ...
        v.addWidget(scroll)
        self._layer_checkboxes = []  # populated by _rebuild_layer_panel_content
    ...
    def _on_loc_updated(self):
        ...
        if self.auto_zoom:
            self.zoom = 1.0               # reset zoom first if requested
            
        # Step 2: optional auto-center on player dot
        if self.auto_center:
            self._center_on_player()      # scroll player dot to center
    ...
    def _apply_z_layer(self, z_value: float):
        ...
        for layer_name, min_z, max_z in ranges:
            ...
            if lo <= z_value <= hi:
                matched_layer = layer_name; break
        ...
        for i, ld in enumerate(layer_defs):
            ...
            if i < len(self.layer_visible) and self.layer_visible[i] != want:
                self.layer_visible[i] = want; changed = True
    ...
    def _do_update_download(self):
        ...
        try:
            ...
            QTimer.singleShot(0, lambda: self.btn_update.setEnabled(True)); return
        ...
        for idx, ent in enumerate(files):
            ...
            if not (dest_abs == base_abs or dest_abs.startswith(base_abs + os.sep)):
                errors.append(f"Unsafe path skipped: {rel}"); continue
        ...
        lines = []
        if remote_ver: lines.append(f"Manifest version: {remote_ver}\n")
        if updated:
            lines.append(f"Downloaded {len(updated)} file(s):\n"
                         + "\n".join(f"  + {r}" for r in updated[:30]))
            if len(updated) > 30: lines.append(f"  ... +{len(updated)-30} more")
        if skipped: lines.append(f"\nSkipped {len(skipped)} already up-to-date file(s).")
        if not updated and not errors: lines.append("Everything is already up to date.")
